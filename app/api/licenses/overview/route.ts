...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      licenseDetails: [
        'Coverage is assigned per active student for the current license year.',
        `Bulk seats purchased: ${snapshot.bulkLicensedStudentCount}. Extra-covered students: ${snapshot.extraCoveredStudents}.`,
        `Annual price per student: ${snapshot.requiredAmountPerStudent.toFixed(2)}.`,
        `Current uncovered students: ${snapshot.uncoveredStudents}. Additional amount to fully cover all active students: ${snapshot.extraLicenseCost.toFixed(2)}.`,
      ],
      consequences: [
        'Coverage is assigned per active student for the current license year.',
        `Bulk seats purchased: ${snapshot.bulkLicensedStudentCount}. Extra-covered students: ${snapshot.extraCoveredStudents}.`,
